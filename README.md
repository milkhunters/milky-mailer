...
| Ключ (для `Consul KV`)        | Описание                                                     | Пример             |
|-------------------------------|--------------------------------------------------------------|--------------------|
| `senders/<senderID>`/         | Уникальный идентификатор отправителя                         | `mailer-one`       |
...
## Отправка сообщений
Для отправки email письма необходимо отправить сообщение в очередь. Название очереди указывается в конфигурации.

**Перед отправкой сообщения необходимо убедиться, что очередь существует.**
...
| `FromId`  | Идентификатор отправителя | `no-reply-base`        |
